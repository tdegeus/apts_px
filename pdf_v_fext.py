...
for seed in range(50):
    ...
    v0 = 100
    w = prrng.pcg32(seed).normal([1000], 1, 0.3)
    vexit = [v0]
...
f = prrng.pcg32().random([w.size, v.size])
...
for i in range(w.size):
    ...
    for j in range(v.size):
        ...
        particle = apts.Quadratic(w=w[i], v0=v[j], f=f[i, j])
...
inset.set_xlim([4e-5, 3e-1])
inset.set_ylim([1e-3, 1e-1])
